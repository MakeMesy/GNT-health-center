...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } else if ($formName == 'delete-ad') {
        $delete_ad_id = $_POST['delete_ad'];
        if ($delete_ad_id) {
            $stmt = $conn->prepare("DELETE from adsbanner where id= ?");
            $stmt->bind_param("s", $delete_ad_id);
            $stmt->execute();
            $stmt->close();
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo ";
              ...
              <form action=./adsbanner.php method='post'>
                            <input type='hidden' name='form_name' value='delete-ad'>
                        <input type='hidden' name='delete_ad' value=$ad_id>
